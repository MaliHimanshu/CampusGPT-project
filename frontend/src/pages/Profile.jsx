  ...
  return (
    <div className="fade-in">
      ...
      <div className="page-body" style={{maxWidth:560}}>
        {/* Avatar card */}
        <div className="card" style={{display:'flex',alignItems:'center',gap:20,marginBottom:16,background:'linear-gradient(135deg, var(--bg-card), var(--bg-elevated))'}}>
          <div className="profile-avatar-lg">{initials}</div>
          <div>
            <h2 style={{fontFamily:'var(--font-display)',fontSize:'1.2rem',fontWeight:800,marginBottom:4}}>{profile?.name||'–'}</h2>
            <p style={{color:'var(--text-secondary)',fontSize:'.875rem',marginBottom:8}}>{profile?.email||'–'}</p>
        ...
        <div className="card" style={{marginBottom:16}}>
          ...
          {[
            ...
            <div key={label} className="info-row">
              ...
              <span className="info-value" style={label==='Account ID'?{fontFamily:'var(--font-mono)',fontSize:'.8rem',color:'var(--text-muted)'}:{}}>{value}</span>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{marginBottom:16}}>
          ...
          {pwErrors.general && (
            <div style={{background:'var(--error-dim)',border:'1px solid rgba(248,113,113,.3)',borderRadius:'var(--radius-md)',padding:'10px 14px',fontSize:'.85rem',color:'var(--error)',marginBottom:16}}>
              ⚠ {pwErrors.general}
            </div>
          )}
        ...
        <div className="card" style={{borderColor:'rgba(248,113,113,.15)'}}>
          ...
          <p style={{fontSize:'.875rem',color:'var(--text-secondary)',marginBottom:14}}>Sign out of your account on this device.</p>
          <button className="btn btn-danger" onClick={() => { localStorage.removeItem('token'); localStorage.removeItem('user'); navigate('/'); }}>
            🚪 Sign out
          </button>
        </div>
      </div>
